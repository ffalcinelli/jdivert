...
public class AsyncCapture {
    public static void main(String[] args) {
        try (WinDivert w = new WinDivert("true")) {
            w.open();
            
            // Start an asynchronous receive operation
            WinDivertAsyncResult<Packet> result = w.recvAsync();
            ...
            if (packet != null) {
                ...
                result.cancel(); // Don't forget to cancel if you stop waiting
            }
        } catch (Exception e) {
            e.printStackTrace();
        }
...
public class SimpleFirewall {
    public static void main(String[] args) {
        ...
        try (WinDivert w = new WinDivert("ip.SrcAddr == 1.2.3.4")) {
            w.open();
